...
function Chat() {
  ...
  return (
    <div className="font w-full h-[85%] flex items-center justify-center overflow-hidden overflow-y-auto px-2 py-1 scroll">
      <div className="w-full lg:w-4/5 flex flex-col h-full items-start justify-start">
        {message?.map((msg, i) => (
          ...
          >
            <img
              src={msg.isBot ? "/icon.png" : "/user.png"}
              alt="user"
              className="w-10 h-10 rounded object-cover"
            />
            <p className="font text-white text-[15px]">{msg?.text}</p>
          </span>
        ))}
        <div ref={msgEnd} />
      </div>
