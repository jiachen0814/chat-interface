...
function LeftNav() {
  ...
  return (
    ...
    >
      ...
      {/* middle section  */}
      <div className="h-[80%] w-full p-2 flex items-start justify-start flex-col overflow-x-auto overflow-y-auto text-sm scroll my-2">
        {/* msg */}
        <span
          className="rounded w-full py-3 px-2 text-xs my-2 flex gap-1 items-center justify-between cursor-pointer hover:bg-gray-800 transition-all duration-300 overflow-x-auto break-words whitespace-normal"
          value={"Give me a short summary of Amy's day."}
          onClick={handleQuery}
        >
          <span className="flex gap-2 items-center justify-center text-base">
            {/* <FiMessageSquare /> */}
            <span className="text-sm">Give me a short summary of Amy's day.</span>
          </span>
        </span>
        <span
          className="rounded w-full py-3 px-2 text-xs my-2 flex gap-2 items-center justify-between cursor-pointer hover:bg-gray-800 transition-all duration-300 overflow-x-auto break-words whitespace-normal"
          value={"How is Amy doing today?"}
          onClick={handleQuery}
        >
          <span className="flex gap-2 items-center justify-center text-base">
            {/* <FiMessageSquare /> */}
            <span className="text-sm">How is Amy doing today?</span>
          </span>
        </span>
      </div>
      {/* bottom section  */}
